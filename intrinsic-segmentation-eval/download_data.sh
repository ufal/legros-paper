...
#    xzcat mn.txt.xz | sacremoses -j 16 -l ${LNG} tokenize -x | sed 's/[[:upper:]]*/\L&/g' > ${LNG}.lc.txt
#    rm mn.txt.xz
#else
#    SIZE=0
#
...
#    done
#
#    zcat *.gz | shuf | head -n 50000000 | sacremoses -j 16 -l ${LNG} tokenize -x | sed 's/[[:upper:]]*/\L&/g' > ${LNG}.lc.txt
#    rm *.gz
#fi

# Get vocabulary from the tokenized corpus
python3 ../../get_vocabulary.py ${LNG}.lc.txt --min-count 5 --num-threads 16 > ${LNG}.lc.txt.vocab
head -n 200000 ${LNG}.lc.txt.vocab > ${LNG}.lc.txt.vocab.200k

cd -

# Train fasttext
python3 ../subword-segmentation/scripts/train_fasttext.py ${LNG}/plaintext/${LNG}.lc.txt ${LNG}/fasttext
